...
fitness = (
  typeCoverage * 0.30 +      // Offensive/defensive type coverage
  avgRankingScore * 0.25 +   // Average across all 4 ranking files
  strategyViability * 0.20 + // ABA/ABB/ABC pattern validity
  metaThreatCoverage * 0.15 + // Coverage of top 50 meta Pokémon
  moveSynergy * 0.10         // Energy breakpoints and move diversity
)
...
if (mode === 'GBL') {
  ...
  fitness -= predictability * 0.10; // Penalty for common cores
}

if (mode === 'PlayPokemon') {
  // Boost consistency in open sheets mode
  fitness += consistency * 0.10;    // Generalist performance
...
offensiveTypes = team.flatMap(pokemon => 
  pokemon.chargedMoves.map(move => 
    typeChart[move.type].superEffective
  )
);
...
const leadScore = rankings.leads[pokemon.speciesId].Score * 0.5 +
                  fastMoveQuality * 0.25 +
                  chargeSpeed * 0.25;

const switchScore = rankings.switches[pokemon.speciesId].Score * 0.4 +
                    bulkScore * 0.3 +
                    safeMatchups * 0.3;

const closerScore = rankings.closers[pokemon.speciesId].Score * 0.5 +
                    sweepPotential * 0.3 +
