...
Runtime simulation availability comes only from the format manifest: build default matrices from each species' active default candidate and exact scenario storage keys, expose active alternatives from the same authority, and validate finite CSV rows plus scenario/default opponent parity before caching. Never enumerate runtime simulation directories or filenames, and never substitute default rows for an unavailable alternate; direct manifest and explicit-variant failures must remain actionable typed errors.
