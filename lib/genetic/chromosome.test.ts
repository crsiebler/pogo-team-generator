...
const dexNumbersBySpeciesId: Record<string, number> = {
  charizard_mega_y: 6,
  palkia_origin: 484,
  eternatus: 890,
  mewtwo: 150,
...
describe('createRandomChromosome legality checks', () => {
  beforeEach(() => {
    vi.clearAllMocks();

    mockPokemonBySpeciesId.mockImplementation((speciesId: string) => ({
      dex: dexNumbersBySpeciesId[speciesId] ?? 999,
      speciesId,
    ...
    mockDexNumber.mockImplementation(
      (speciesId: string) => dexNumbersBySpeciesId[speciesId],
    );
