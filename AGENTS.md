...
- Follow the refreshed optimizer diagnostics contract in `docs/pokemon-go-team-optimization.md` and `docs/team-optimization/`: Summary Statistics is display-only and uses simple `A`, `B`, `C`, `D`, or `F` grades with no plus/minus modifiers or quality pills; Threat Score is a lower-is-better diagnostic from `scoreBreakdown.threatScore`; Recommended Lineups use blue diagnostic styling and one API-provided textual quality pill without exposing numeric lineup scores.
